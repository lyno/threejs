...
/**
 * 欧拉角这种旋转方式是最直观的，因为这种方式是将旋转表示为物体按坐标系的三个轴X(1,0,0) ,Y(0,1,0),Z(0,0,1)的旋转组合成的。这里首先要明确两个概念，
 * 1参考系：类似于物理中的参考系，是静止不动的，比如北极星，不管在那里，那就是北。
 * 2坐标系：坐标系是固定于物体的，随着物体的转动而发生变化，最简单的例子就是左右，人所说的左边一直是根据人所面对的方向来决定的。在下图中蓝色为参考系的三个轴，而红色是物体的坐标系的三个轴。
 * 虽然说欧拉角表示的旋转是有多个沿坐标轴的旋转组合而成的。但是旋转的顺序不同旋转就不同，所以，欧拉角中旋转的顺序要注明。这里的三个角是zxz顺规的欧拉角。物体先绕Z轴旋转了α°，然后物体坐标系的x轴的位置变化到了图中N的位置，然后绕这个N轴(X轴)旋转β°，最后再沿Z轴旋转γ°。
 * http://js.soufunimg.com/upload/vr/three/euler.png
 * 表示将一个几何体绕x轴旋转x度，绕y轴旋转y度，绕z轴旋转z度;并且旋转的顺序是xyz ; 第三个参数旋转顺序可以是'XYZ', 'YZX', 'ZXY', 'XZY', 'YXZ', 'ZYX'
 * 欧拉角和旋转矩阵相互转换 https://blog.csdn.net/Darlingqiang/article/details/80829666
 * @param {*} x
 * @param {*} y
 * @param {*} z
 * @param {*} order
